...
const queryWatsonToneHelper = (songString) =>
  tone_analyzer.toneAsync({ text: songString })
  .then(data => {
    const emotionalTone = data.document_tone.tone_categories[0];
    const languageTone = data.document_tone.tone_categories[1];
    const socialTone = data.document_tone.tone_categories[2];
    return {
      song: songString,
      // Emotion Tone
